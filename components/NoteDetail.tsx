...
function extractKeyTopics(transcript: string): string[] {
  ...
  return Object.entries(freq).sort((a, b) => b[1] - a[1]).slice(0, 6).map(([word]) => word);
}
...
export function NoteDetail({ note, userId, allNotes, onBack, onDelete, onSelectNote }: NoteDetailProps) {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div className="lg:col-span-8 space-y-8">
              <section>
                ...
                <h1 className="text-4xl md:text-5xl font-extrabold text-primary leading-tight mb-4" style={{ fontFamily: "Manrope, sans-serif" }}>Voice Recording</h1>
                <div className="flex items-center gap-2 text-on-surface-variant">
                  <span className="material-symbols-outlined text-lg">calendar_today</span>
                  <span className="text-sm font-semibold">{formatFullDate(note.createdAt)} at {formatTimeOfDay(note.createdAt)}</span>
                </div>
              </section>
              ...
              {summary && (
                <section className="bg-primary/5 rounded-3xl p-8 border border-primary/10">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="font-extrabold text-lg text-primary" style={{ fontFamily: "Manrope, sans-serif" }}>AI Insights Summary</h3>
                  </div>
              ...
              <section>
                ...
                <div className="group flex gap-4 p-4 rounded-2xl hover:bg-surface-container-low transition-colors">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-lg leading-relaxed text-on-surface group-hover:text-primary transition-colors">{note.transcript}</p>
                  </div>
                </div>
              </section>
            </div>
            ...
            <div className="lg:col-span-4 space-y-6">
              {keyTopics.length > 0 && (
                <div className="bg-white border border-outline-variant/50 rounded-3xl p-6">
                  <h4 className="font-extrabold text-lg text-primary mb-4" style={{ fontFamily: "Manrope, sans-serif" }}>Key Topics</h4>
                  <div className="flex flex-wrap gap-2">
                    {keyTopics.map((topic) => (
                      <span key={topic} className="px-4 py-2 rounded-2xl bg-surface-container-low text-primary text-sm font-semibold border border-outline-variant/30 hover:bg-primary-container transition-colors">#{topic}</span>
                    ))}
                  </div>
                </div>
              )}
